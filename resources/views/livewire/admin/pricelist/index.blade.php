<div class="space-y-6">
    ...
    <div
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Gestión de Lista de Precios</h1>
            {{--   <p class="text-gray-500 text-sm">Administra cuentas y roles del sistema</p> --}}
        </div>
        <div class="flex flex-wrap gap-2">
            <!-- Nuevo -->
            <a href="{{ route('admin.pricelists.create') }}"
                ...
                <i class="fa-solid fa-plus mr-2"></i> Nuevo Lista de Precio
            </a>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 flex flex-wrap gap-4 justify-between items-center">
        ...
        <div class="relative w-full md:w-1/3">
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Buscar lista de precios..."
                ...
                @forelse ($pricelists as $pricelist)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        ...
                        <td class="px-4 py-3 text-center">
                            <input type="checkbox" wire:model.live="selectedUsers.{{ $pricelist->id }}"
                                wire:key="user-checkbox-{{ $pricelist->id }}">
                        </td>

                        <!-- ID -->
                        <td class="px-4 py-3 text-center">{{ $pricelist->id }}</td>

                        <!-- Nombre con resaltado -->
                        <td class="px-4 py-3 text-center">
                            <a href="{{ route('admin.pricelists.items', $pricelist) }}"
                        ...
                        <td class="px-4 py-3 text-center">
                            @if ($pricelist->state)
                                <span wire:click="toggleStatus({{ $pricelist->id }})"
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex justify-center gap-2">
                                ...
                                <!-- Ver -->
                                <a href="{{ route('admin.users.show', $pricelist) }}"
                                ...
                                <!-- Editar -->
                                <a href="{{ route('admin.users.edit', $pricelist) }}"
                                ...
                                <!-- Eliminar -->
                                {{-- <button onclick="confirmDeleteSingle({{ $pricelist->id }})" --}}
                                <button
                                    onclick="confirmDeletesimple({{ $pricelist->id }}, @js($pricelist->name), 'deleteSingle', 'Este usuario {{ $pricelist->name }} con ID {{ $pricelist->id }} será eliminado.')"
