<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-5">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Editar diario</h1>
                <div class="flex items-center gap-3 mt-1">
                    <span class="px-2 py-1 rounded bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 text-sm font-semibold">
                        {{ $record->code }}
                    </span>
                    <span class="text-sm text-gray-500 dark:text-gray-300">{{ $record->name }}</span>
                    @if ($record->journalType)
                        <span class="px-2 py-0.5 rounded bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300 text-xs">
                            {{ $record->journalType->name }}
                        </span>
                    @endif
                </div>
            </div>
        ...
        <div class="p-6 space-y-6">
                ...
                <div class="md:col-span-12">
                    <div class="rounded-xl border border-sky-200 dark:border-sky-800 bg-sky-50 dark:bg-sky-900/20 p-4">
                        <p class="text-sm text-sky-700 dark:text-sky-300">
                            ...
                            Las cuentas son opcionales y dependen del tipo de diario.
                        </p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-5">
                ...
                <div class="md:col-span-4">
                    <label class="{{ $labelBase }}">CCI</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-5">
                ...
                <div class="md:col-span-3">
                    ...
                    <input wire:model.defer="document_prefix" type="text" placeholder="Ej: F001, B001..."
                        class="{{ $controlBase }} mt-2 uppercase">
                    <p class="text-xs text-gray-500 dark:text-gray-300 mt-1">Serie para facturación electrónica.</p>
                    @error('document_prefix') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="md:col-span-3">
                    <label class="{{ $labelBase }}">Número siguiente <span class="text-red-500">*</span></label>
            ...
            <div class="pt-2 flex justify-end border-t border-gray-200 dark:border-gray-700">
                <button wire:click="update"
                    class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl
                           bg-gradient-to-r from-indigo-600 to-sky-600 hover:from-indigo-700 hover:to-sky-700
                           text-white font-semibold shadow-sm transition">
                    <i class="fa-regular fa-floppy-disk"></i> Actualizar diario
                </button>
            </div>
            @endcan
