<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-5 flex justify-between items-center">
        ...
        <a href="{{ route('purchase.order.create') }}"
            class="bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-indigo-700 transition">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow overflow-hidden">
        <table class="w-full text-left">
            <thead>
                <tr
                    class="bg-gray-50 dark:bg-gray-900/50 text-[11px] font-black text-gray-500 uppercase tracking-wider border-b">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="p-4 text-center">
                            <div class="flex justify-center gap-2">
                                ...
                                {{-- 🚀 NUEVO: Botón Editar/Ver (Odoo Style) --}}
                                <a href="{{ route('purchase.order.edit', $order->id) }}"
                                    class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition"
                                    title="Editar / Confirmar">
                                    <i class="fa-solid fa-pen-to-square text-lg"></i>
                                </a>
                                {{-- PDF --}}
                                <button wire:click="viewPdf({{ $order->id }})"
                                    class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition" title="Ver PDF">
                                    <i class="fa-solid fa-file-pdf text-lg"></i>
                                </button>

                                {{-- WhatsApp --}}
                                <button wire:click="sendWhatsApp({{ $order->id }})"
                                ...
                                {{-- Email --}}
                                <button wire:click="sendEmail({{ $order->id }})"
