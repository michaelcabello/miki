    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($product_templates as $product_template)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        <!-- Checkbox -->
                        <td class="px-4 py-3 text-center">
                            {{-- <input type="checkbox" wire:model.live="selectedUsers.{{ $product_template->id }}"
                                wire:key="user-checkbox-{{ $product_template->id }}"> --}}
                        </td>

                        <!-- ID -->
                        <td class="px-4 py-3 text-center">{{ $product_template->id }}</td>

                        <!-- Nombre con resaltado -->
                        <td class="px-4 py-3 text-center">
                            <a {{-- href="{{ route('admin.pricelists.items', $product_template) }}" --}} class="text-blue-600 hover:underline">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if ($product_template->non_default_variants_count > 0)
                                <a href="{{ route('admin.products.variants', $product_template) }}"
                                    class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full
                                             ...
                                             hover:bg-indigo-100 transition text-xs font-semibold">
                                    <i class="fa-solid fa-cubes-stacked"></i>
                                    Ver variantes ({{ $product_template->variants_count }})
                                </a>
                            @else
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex justify-center gap-2">
                                ...
                                <!-- Eliminar -->
                                {{-- <button onclick="confirmDeleteSingle({{ $product_template->id }})" --}}
                                <button
                                    onclick="confirmDeletesimple({{ $product_template->id }}, @js($product_template->name), 'deleteSingle', 'Este usuario {{ $product_template->name }} con ID {{ $product_template->id }} será eliminado.')"
